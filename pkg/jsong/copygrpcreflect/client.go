package copygrpcreflect

// 从jhump/protoreflect v1.6.0中拷出
// 考虑把所有相关代码都从依赖里抽出来
import (
	"bytes"
	"fmt"
...
func (cr *Client) FileByFilename(filename string) (*desc.FileDescriptor, error) {
	...
	if isNotFound(err) {
		...
		if alternate, ok := StdFileAliases[filename]; ok {
			req := &rpb.ServerReflectionRequest{
				MessageRequest: &rpb.ServerReflectionRequest_FileByFilename{
...
func (cr *Client) CleanCache() {
	cr.cacheMu.Lock()
	cr.protosByName = map[string]*dpb.FileDescriptorProto{}
	cr.filesByName = map[string]*desc.FileDescriptor{}
	cr.filesBySymbol = map[string]*desc.FileDescriptor{}
	cr.filesByExtension = map[extDesc]*desc.FileDescriptor{}
	cr.cacheMu.Unlock()
}
